...
public interface SurgeryApi {
    ...
    ResponseEntity<List<SurgeryResult>> findAllSurgeries();
    ...
    @ApiResponses(value = {@ApiResponse(code = 200, message = "Fetched", response = Surgery.class)})
    ResponseEntity<SurgeryResult> findSurgeryById(UUID id) throws Exception;

    @ApiOperation(value = "create Surgery", notes = "This operation will create a new surgery", response = Surgery.class, tags = {"Surgery API"})
    @ApiResponses(value = {@ApiResponse(code = 201, message = "Created", response = SurgeryResult.class)})
    ResponseEntity<SurgeryResult> addSurgery(SurgeryRequest request);
    ...
    ResponseEntity<SurgeryResult> updateSurgery(UUID id, SurgeryRequest request);
